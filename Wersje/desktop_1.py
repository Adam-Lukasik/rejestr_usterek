...
# UI v2 — zakładki (Opcja C):
#   • Zakładka "Lista usterek"  — tabela pełna szerokość + panel szczegółów
#   • Zakładka "Dodaj usterkę" — przestronny formularz dwukolumnowy
#   • Splash, Dark mode, zmaksymalizowane okno
#   • Ostatnio wybrany klient zapamiętywany w desktop_config.json

import os, sys, json, sqlite3, threading, csv
...
class App:
    ...
    def _build_list_tab(self, parent):
        ...
        # panel szczegółów
        detail = ttk.Labelframe(parent, text="Szczegóły", padding=12)
        detail.grid(row=1, column=2, sticky="nsew", padx=(10, 0))
        parent.columnconfigure(2, weight=0, minsize=280)

        def _lbl(text):
            ttk.Label(detail, text=text, font=("Segoe UI", 8, "bold"),
    ...
    def _build_form_tab(self, parent):
        ...
        # prawa: opisy
        right = ttk.Labelframe(parent, text="Opisy", padding=14)
        ...
        for r in rows:
            self._tree.insert("", "end", iid=r["id"], tags=(r.get("status","open"),),
                values=(self._fmt_dt(r.get("created","")),
                        ...
                        snip(r.get("opisProblem","")),
                        snip(r.get("opisNaprawa","")),
    ...
    def _save_form(self):
        ...
        try:
            if self._edit_id:
                API.put(f"/api/records/{self._edit_id}", payload)
            else:
    ...
    def _delete_selected(self):
        r = self._selected()
        if not r: messagebox.showinfo(APP_TITLE,"Zaznacz usterkę."); return
        if not messagebox.askyesno(APP_TITLE,"Usunąć tę usterkę? Operacja jest nieodwracalna."): return
